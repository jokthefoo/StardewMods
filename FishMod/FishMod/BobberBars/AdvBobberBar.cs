	...
	public class AdvBobberBar : IClickableMenu
	{
		...
		public override void update(GameTime time)
		{
			...
			else
			{
				...
				if (bobberPosition >= (float)(548 - bobberBarHeight) &&
				...
				if (buttonPressed && gravity < 0f &&
				    (bobberBarPos == 0f || bobberBarPos == (float)(568 - bobberBarHeight)))
				{
					bobberBarSpeed = 0f;
				}
				...
				if (bobberBarPos + (float)bobberBarHeight > 568f)
				{
					...
					bobberBarSpeed = (0f - bobberBarSpeed) * 2f / 3f * (bobbers.Contains("(O)692") // lead bobber prevents bounce
						? ((float)Utility.getStringCountInList(bobbers, "(O)692") * 0.1f)
						: 1f);
		...
		public override void draw(SpriteBatch b)
		{
			...
			if (scale == 1f)
			{
				...
				// The Fish
				b.Draw(Game1.mouseCursors,
					new Vector2(xPositionOnScreen + 64 + 18, (float)(yPositionOnScreen + 12 + 24) + bobberPosition) +
